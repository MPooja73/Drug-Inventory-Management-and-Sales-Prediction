...
const UserSignup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      await axios.post("/auth/user/signup", form);
      navigate("/user/login");
      // Show success message on the next page instead of an alert
    } catch (err) {
      setError(
      ...
      <div className="w-full max-w-md px-6 py-8">
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-700 px-6 py-4">
            <h2 className="text-2xl font-bold text-white text-center">Create User Account</h2>
            <p className="text-blue-100 text-center text-sm mt-1">
              Register for MediTrack user access
            </p>
          </div>
        ...
        <div className="flex justify-center mt-8">
          <p className="text-gray-500 text-sm text-center">
            MediTrack Pharmacy Solutions - Secure User Registration
          </p>
        </div>
      </div>
    </div>
